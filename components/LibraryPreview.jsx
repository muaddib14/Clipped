...
const FOLDERS = [
  {
    name: 'SORA Research',
    clips: [
      { title: 'How Sora Generates Video from Text', domain: 'openai.com', date: '2026-07-08' },
      { title: 'Diffusion Models Explained', domain: 'arxiv.org', date: '2026-07-06' },
      { title: 'The Future of Text-to-Video', domain: 'theverge.com', date: '2026-07-02' },
    ],
  },
  {
    name: 'paulgraham.com',
    clips: [
      { title: 'How to Do Great Work', domain: 'paulgraham.com', date: '2026-06-28' },
      { title: 'What You Cant Say', domain: 'paulgraham.com', date: '2026-06-20' },
    ],
  },
  {
    name: 'Product Notes',
    clips: [
      { title: 'Designing for Trust', domain: 'medium.com', date: '2026-07-01' },
      { title: 'The Case for Boring Software', domain: 'increment.com', date: '2026-06-25' },
    ],
  },
  {
    name: 'medium.com',
    clips: [
      { title: 'Why Writers Should Learn to Code', domain: 'medium.com', date: '2026-06-30' },
    ],
  },
];
...
function ClipRow({ title, domain, date }) {
...
export function LibraryPreview() {
  ...
  const filteredFolders = q
    ? FOLDERS.filter(
        (f) => f.name.toLowerCase().includes(q) || f.clips.some((c) => c.title.toLowerCase().includes(q))
      )
    : FOLDERS;
  const filteredClips = q
    ? allClips.filter((c) => c.title.toLowerCase().includes(q) || c.domain.toLowerCase().includes(q))
    : allClips;
          ...
          <>
            <div className="mb-6">
              ...
              <p className="font-mono text-[11px]" style={{ color: 'rgba(245,240,232,0.45)' }}>
                Organize and manage your saved articles
              </p>
            </div>
            ...
            {view === 'folders' ? (
              ...
              <div className="space-y-2">
                {filteredClips.map((c, i) => (
                  <ClipRow key={i} title={c.title} domain={c.domain} date={c.date} />
                ))}
              </div>
            ) : (
              <p className="font-mono text-[12px] text-center py-12" style={{ color: 'rgba(245,240,232,0.4)' }}>
